...
void entryPoint(const char* address, long double* latitude, long double* longitude, long double* imageDirection,long double* altitude,char rotation)
{
    ExifData *ed;
    ed = exif_data_new_from_file(address);
    ...
    exif_entry_get_value(orient, orientation,100);

    printf("value of orientaion is %s",orientation );
